...
export default {
  schema: {
    ...
    description: "Get the billing information for the GCP project — billing account name and whether billing is enabled",
    destructive: false,
    inputSchema: {
      type: "object",
      properties: {},
  ...
  async execute(_input) {
    ...
    try {
      ...
      const url   = `https://cloudbilling.googleapis.com/v1/projects/${GCP_PROJECT_ID}/billingInfo`;
      const data  = await gcpFetch(token, url);

      const lines = [
        `Project:         ${data.name || GCP_PROJECT_ID}`,
        `Billing Account: ${data.billingAccountName || "(none)"}`,
        `Billing Enabled: ${data.billingEnabled ? "true" : "false"}`,
      ];
