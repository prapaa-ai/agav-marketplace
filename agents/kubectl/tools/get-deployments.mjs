...
export default {
  schema: {
    ...
    description: "List Kubernetes deployments with ready, up-to-date, and available replica counts",
    destructive: false,
    inputSchema: {
      type: "object",
      properties: {
        namespace: { type: "string", description: "Namespace to list deployments in (omit for default namespace)" },
  ...
  async execute(input) {
    ...
    if (!result.stdout) {
      return { output: "No deployments found.", isError: false };
    }
