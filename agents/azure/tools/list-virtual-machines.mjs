...
export default {
  schema: {
    ...
    description: "List Azure Virtual Machines in the subscription, optionally scoped by resource group",
    destructive: false,
    inputSchema: {
      type: "object",
      properties: {
  ...
  async execute(input) {
    ...
    try {
      ...
      const path = input.resource_group
        ? `/resourceGroups/${encodeURIComponent(input.resource_group)}/providers/Microsoft.Compute/virtualMachines`
        : "/providers/Microsoft.Compute/virtualMachines";

      const data = await azureFetch(token, `${base}${path}?api-version=2023-03-01`);
      const vms  = data.value || [];

      if (vms.length === 0) {
        return { output: "No virtual machines found.", isError: false };
      }

      const lines = vms.map(vm => {
        const size     = vm.properties?.hardwareProfile?.vmSize || "unknown";
        const osType   = vm.properties?.storageProfile?.osDisk?.osType || "unknown";
        const location = vm.location || "unknown";
        return `${vm.name} — size: ${size}, location: ${location}, os: ${osType}`;
      });

      return { output: `Virtual Machines (${lines.length}):\n${lines.join("\n")}`, isError: false };
    } catch (err) {
